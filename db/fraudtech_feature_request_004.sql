...
CREATE TABLE IF NOT EXISTS `fraudtech_feature_request`.`feature_request` (
  ...
  PRIMARY KEY (`FRid`),
  UNIQUE INDEX `FRid_UNIQUE` (`FRid` ASC) VISIBLE,
  INDEX `FR_LOGid_idx` (`LOGid` ASC) VISIBLE,
  INDEX `FR_TEid_idx` (`TEid` ASC) VISIBLE,
  CONSTRAINT `FR_LOGid`
    FOREIGN KEY (`LOGid`)
...
CREATE TABLE IF NOT EXISTS `fraudtech_feature_request`.`feature` (
  ...
  `notes` TEXT NULL,
  PRIMARY KEY (`FEid`),
  UNIQUE INDEX `FEid_UNIQUE` (`FEid` ASC) VISIBLE,
  INDEX `FE_FRid_idx` (`FRid` ASC) VISIBLE,
  CONSTRAINT `FE_FRid`
    FOREIGN KEY (`FRid`)
...
CREATE TABLE IF NOT EXISTS `fraudtech_feature_request`.`stakeholders` (
  ...
  INDEX `SH_LOGid_idx` (`LOGid` ASC) VISIBLE,
  INDEX `SH_FRid_idx` (`FRid` ASC) VISIBLE,
  CONSTRAINT `SH_LOGid`
    FOREIGN KEY (`LOGid`)
    REFERENCES `fraudtech_feature_request`.`people` (`LOGid`)
    ON DELETE NO ACTION
...
CREATE TABLE IF NOT EXISTS `fraudtech_feature_request`.`team_membership` (
  ...
  INDEX `TM_TEid_idx` (`TEid` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `fraudtech_feature_request`.`stakeholder_discussions` (
  ...
  INDEX `SD_SHid_idx` (`SHid` ASC) VISIBLE,
  INDEX `SD_FRid_idx` (`FRid` ASC) VISIBLE,
  CONSTRAINT `SD_SHid`
    FOREIGN KEY (`SHid`)
